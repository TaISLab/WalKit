...
HandlePublisher::HandlePublisher() : Node("handle_publisher"){
        ...
        */
        ...
        publisher_ = this->create_publisher<walker2_msgs::msg::W2Hand>(topic_name_, 10);
...
void HandlePublisher::start(){
        ...
        std::stringstream ss;
        std::string data;
        walker2_msgs::msg::W2Hand measurement;
...
std::string HandlePublisher::get_serial_port(std::string preamble, int highest_port) {
        ...
        for (int i=0;i<=highest_port;i++){
            ...
            // Set the baud rate of the serial port.
            my_serial_stream.SetBaudRate(BaudRate::BAUD_115200) ;
            // Set the number of data bits.
            ...
            while( (my_serial_stream.rdbuf()->in_avail() == 0) && (attempts<max_attempts) ){
                usleep(1000);
